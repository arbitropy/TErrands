...
public class MainBoardController implements Initializable {
    ...
    void collaborateAddTextFieldAction(ActionEvent event) throws SQLException, IOException, CsvValidationException {
        ...
        //     return;
        // }
        String[] match = Main.MatchCreds(email);
        if(match == null){
            source.clear();
            source.setPromptText("Invalid user");
            source.setStyle("-fx-background-color:"+ Main.colors.get("Wrong")+"; -fx-background-radius: 20");
        ...
        // statement.executeUpdate();
        Main.AddProject(email, fileName);
